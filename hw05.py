...
class Game:
    ...
    def play(self):
        while not self.game_over():
            ...
            Player.choose(curr, other)(other)
            self.turn += 1
        return self.winner()
...
def deep_map_mut(func, lnk):
    ...
    if lnk is Link.empty:
        return
    elif isinstance(lnk.first, Link): # to ensure that lnk.first is an element, not a linked list itself
        deep_map_mut(func, lnk.first)
    else: # only apply func if lnk.first is an element/value
